...
class BaseGraspDataset(torch.utils.data.Dataset):
    def __init__(self):
        # self.pointnetgpd_dir = os.environ["PointNetGPD_FOLDER"]
        self.transform = pickle.load(open(f"data/pointnetgpd/google2cloud.pkl", "rb"))
    ...
    def collect_pc(self, grasp, pc, transform):
        ...
        minor_normal = (np.dot(transform, np.array([minor_normal[0], minor_normal[1], minor_normal[2], 0])))[
                       :3].reshape(3, 1)
        matrix = np.hstack([approach, binormal, minor_normal]).T # transform matrix from pcd to hand
    ...
    def get_normal(self, points, radius=0.1, max_nn=30):
        """
    ...
    def project_pc(self, pc, gripper_width):
        ...
        """
        # norm.set_KSearch(self.normal_K) this value was set to 10 when using python-pcl
...
class PointGraspDataset(BaseGraspDataset):
    def __init__(self, obj_points_num, grasp_points_num, pc_file_used_num, grasp_amount_per_file, thresh_good,
        ...
        fl_grasp = glob.glob(f"{self.pointnetgpd_dir}/PointNetGPD/data/ycb_grasp/{self.tag}/*.npy")
...
class PointGraspOneViewDataset(BaseGraspDataset):
    def __init__(self, grasp_points_num, grasp_amount_per_file, thresh_good,
        ...
        fl_grasp = glob.glob(f"data/pointnetgpd/ycb_grasp/{self.tag}/003_cracker_box.npy")
        fl_pc = glob.glob(f"data/pointnetgpd/ycb-tools/models/ycb/003_cracker_box/rgbd/clouds/pc_NP3_NP5*.npy")
        self.d_pc, self.d_grasp = {}, {} # d_pc: key is model name, value is a list of single view point cloud
        for i in fl_pc: # i: .../003_cracker_box/rgbd/clouds/pc_NP3_NP5_000.npy. point cloud numpy format
            k = i.split('/')[-4] # k: 003_cracker_box
            if k in self.d_pc.keys():
                self.d_pc[k].append(i)
            else:
                self.d_pc[k] = [i]
        ...
        for i in fl_grasp: # .../data/ycb_grasp/train/003_cracker_box.npy. grasp pose of single model with numpy format
            k = i.split('/')[-1].split('.')[0] # k: 003_cracker_box
            self.d_grasp[k] = i
        object1 = set(self.d_grasp.keys())
        object2 = set(self.transform.keys())
        self.object = list(object1.intersection(object2))
    ...
    def __getitem__(self, index):
        # random one grasp of any object, index is the random grasp index
        obj_ind, grasp_ind = np.unravel_index(index, (len(self.object), self.grasp_amount_per_file)) 

        obj_grasp = self.object[obj_ind]
        obj_pc = self.transform[obj_grasp][0]
        f_grasp = self.d_grasp[obj_grasp]
        fl_pc = np.array(self.d_pc[obj_pc]) # all single view point cloud file path of random object
        np.random.shuffle(fl_pc)

        grasp = np.load(f_grasp)[grasp_ind] # grasp pose of random object
        pc = np.load(fl_pc[-1]) # random select one single view pcd
        t = self.transform[obj_grasp][1]
...
class OneViewObjectPointDataset(torch.utils.data.Dataset):
    
    def __init__(self):
        self.pointnetgpd_dir = os.environ["PointNetGPD_FOLDER"]
        fl_pc = glob.glob(f"{self.pointnetgpd_dir}/data/ycb-tools/models/ycb/003_cracker_box/rgbd/clouds/pc_NP3_NP5*.npy")
        
        self.d_pc = {} # d_pc: key is model name, value is a list of single view point cloud
        for i in fl_pc: # i: .../003_cracker_box/rgbd/clouds/pc_NP3_NP5_000.npy. point cloud numpy format
            k = i.split('/')[-4] # k: 003_cracker_box
            if k in self.d_pc.keys():
                self.d_pc[k].append(i)
    ...
    def __getitem__(self, index):
        ...
        fl_pc = np.array(self.d_pc["003_cracker_box"]) # all single view point cloud file path of one object
        pc = np.load(fl_pc[index]) # random select one single view pcd
        
        if len(pc) > 10000:
            pc = pc[np.random.choice(len(pc), size=10000, replace=False)].T
        else:
            pc = pc[np.random.choice(len(pc), size=10000, replace=True)].T
